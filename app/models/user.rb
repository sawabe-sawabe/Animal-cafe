class User < ApplicationRecord
  ...
  has_many :followings, through: :followers, source: :followed # 中間テーブルのfollowersを通り、Userモデルのfollowedインスタンスを取得するため
  has_many :followers, through: :followings, source: :follower # 中間テーブルのfollowingsを通り、Userモデルのfollowerインスタンスを取得するため
  attachment :image
  def follow(user_id)
